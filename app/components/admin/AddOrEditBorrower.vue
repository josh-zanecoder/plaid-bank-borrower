...
const handleSubmit = async () => {
  ...
  try {
    if (isEditMode.value) {
      const response = await $fetch<{ success: boolean; message?: string; data?: any }>(`/api/admin/borrower/${props.borrower?.id || props.borrower?._id}`, {
        method: 'PUT',
        body: {
      ...
      const response = await $fetch<{ success: boolean; message?: string; data?: any }>('/api/admin/borrower/create', {
        method: 'POST',
        body: {
          firstName: form.firstName.trim(),
          lastName: form.lastName.trim(),

          ...
          <div class="bg-white px-6 pt-6 pb-6 sm:p-8">
            ...
            <form @submit.prevent="handleSubmit" class="space-y-5">
              ...
              <div class="flex items-center justify-end gap-3 pt-4">
                ...
                >
                  <span v-if="!loading">{{ isEditMode ? 'Update Bank' : 'Create Bank' }}</span>
...
const handleSubmit = async () => {
  ...
  try {
    if (isEditMode.value) {
      const response = await $fetch<{ success: boolean; message?: string; data?: any }>(`/api/admin/bank/${props.bank?.id || props.bank?._id}`, {
        method: 'PUT',
        body: {
          name: form.name.trim(),
      ...
      if (response.success) {
        success.value = response.message || 'Bank updated successfully!'
        setTimeout(() => {
          resetForm()
          emit('updated')
          emit('close')
      ...
      const response = await $fetch<{ success: boolean; message?: string; data?: any }>('/api/admin/bank/create', {
        method: 'POST',
        body: {
          name: form.name.trim(),
          email: form.email.trim(),
      ...
      if (response.success) {
        success.value = response.message || 'Bank created successfully!'
        setTimeout(() => {
          resetForm()
          emit('created')
          emit('close')
    ...
    error.value = err.data?.statusMessage || err.message || (isEditMode.value ? 'Failed to update bank. Please try again.' : 'Failed to create bank. Please try again.')
  } finally {
    loading.value = false
  }
}
...
watch(() => props.bank, (newBank) => {
  if (newBank && props.isOpen) {
    form.name = newBank.name || ''
    form.email = newBank.email || ''
    form.password = ''
  }
})
</script>
